...
function FooterItems({dropdownNumber, activeDropdownNumber, footerName, footerNavItemsArray, handleClick}){
  ...
  if(activeDropdownNumber === dropdownNumber){
    return(
      <div onClick = {() => handleClick(dropdownNumber)} className = {styles["col"]}>
        <h3>{footerName}<i className="fa-solid fa-caret-right"></i></h3>
        {footerNavItemsArray.length > 0 && 
        <ul>
          {footerNavItemsArray.map((value) => (
            <li key = {value.id}><Link to = {value.url}>{value.name}</Link></li>
          ))}
        </ul>}
  ...
  return(
    <div onClick = {() => handleClick(dropdownNumber)} className = {`${styles["col"]} ${styles['mobile-collapsed']} `}>
      <h3>{footerName}<i className="fa-solid fa-caret-down"></i></h3>
      {footerNavItemsArray.length > 0 && 
      <ul>
        {footerNavItemsArray.map((value) => (
          <li key = {value.id}><Link to = {value.url}>{value.name}</Link></li>
