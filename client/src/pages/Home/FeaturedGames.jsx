...
function FeaturedGames({games}){
  ...
  function createCircleGameNav(index, key){
    if(activeGameNumber === index) {
      return (<button aria-label = "circle-nav" key = {key} className = {`${styles.active} ${styles["circle-nav"]}`}></button>)
    }
    return (<button aria-label = "circle-nav" key = {key} onClick = {() => {handleCircleClick(index)}} className = {styles["circle-nav"]}></button>)
  }
